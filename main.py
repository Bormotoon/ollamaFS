...
class DocumentSorter:
	...
	async def async_generate_auto_categories(self, files):
		...
		# Тестовый запрос к Ollama с коротким ответом
		logger.info("Sending test request to Ollama to check connectivity")
		async with aiohttp.ClientSession() as session:
			try:
				async with session.post(f"{self.ollama_url}/generate",
										json={
											"model": self.model,
											"prompt": "Respond with 'OK' only to test connectivity.",
											"stream": False,
											"num_predict": 2  # Ограничиваем до 2 токенов (~"OK")
										},
										timeout=aiohttp.ClientTimeout(total=10)) as test_response:
					logger.info(f"Test request status: {test_response.status}")
					if test_response.status == 200:
						test_data = await test_response.json()
						logger.info(f"Test response: {test_data}")
						if test_data.get("response") != "OK":
							logger.warning(f"Unexpected test response: {test_data}")
					else:
						logger.warning(f"Test request failed with status: {test_response.status}")
			except Exception as e:
				import traceback
				logger.error(f"Test request failed: {str(e)}\n{traceback.format_exc()}")

		# Основная обработка батчей
		batch_size = 10
		categories = {}
		...
		for i in range(0, len(files), batch_size):
			...
			file_info_list = [{"filename": os.path.basename(f), "extension": os.path.splitext(f)[1].lower(),
							   "size_bytes": os.path.getsize(f)} for f in batch]
			prompt = f"""
	        {_('Analyze the following files and suggest a hierarchical category structure with a maximum depth of')} {self.max_depth}.
	        {_('Return a JSON object with categories and subcategories based on file names, extensions, and sizes. Keep it concise.')}
			...
			logger.info(
				f"Sending batch {i // batch_size + 1} of {len(batch)} files to Ollama (prompt size: {prompt_size} bytes)")
			async with aiohttp.ClientSession() as session:
				try:
					async with session.post(f"{self.ollama_url}/generate",
											json={
												...
												"prompt": prompt,
												"stream": False,
												"num_predict": 100  # Ограничиваем до 100 токенов для компактного JSON
											},
											timeout=aiohttp.ClientTimeout(total=60)) as response:
						logger.info(f"Ollama response status for batch {i // batch_size + 1}: {response.status}")
						if response.status == 200:
							data = await response.json()
							logger.debug(f"Ollama response for batch {i // batch_size + 1}: {data}")
							batch_categories = json.loads(data.get("response", "{}"))
							for cat, subcats in batch_categories.items():
								if cat not in categories:
									categories[cat] = {}
								categories[cat].update(subcats)
						else:
							logger.warning(
								_(f"Failed to generate categories for batch {i // batch_size + 1}: {response.status}"))
				except Exception as e:
					import traceback
					logger.error(
						_(f"Error generating categories for batch {i // batch_size + 1}: {str(e)}\n{traceback.format_exc()}"))
		if categories:
			...
			logger.warning(_("No categories generated. Using default categories."))
			self.category_list = ["Documents", "Images", "Audio", "Other"]
		...
		try:
			...
			if self.auto_sort_var.get():  # Если включена автоматическая сортировка
				try:
					...
					logger.warning(_(f"Auto-category generation failed: {error_details}. Using default categories."))
